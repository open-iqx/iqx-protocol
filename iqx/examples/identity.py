...
# An agent id that is already usable as a filename verbatim. Ids matching this
# keep their historical credential filename, so upgrading does not orphan an
# existing key. See key_path_for.
_FILENAME_SAFE_ID = re.compile(r"[A-Za-z0-9._-]{1,128}")

# Separates the readable prefix from the digest in a derived filename.
# Deliberately OUTSIDE the safe charset above: that is what makes a derived
# name unable to collide with a safe id's name.
_DERIVED_MARKER = "~"

# How much of the sanitized id to keep for readability, and how many hex
# characters of SHA-256 to append. 32 hex characters is 128 bits — enough that
# two distinct ids sharing a credential file is not a case worth handling.
_DERIVED_PREFIX_LEN = 64
_DERIVED_DIGEST_CHARS = 32
...
def key_path_for(agent_id: str, *, state_dir: Optional[Path] = None) -> Path:
    ...
    Derived from the id rather than fixed per example: identities are
    ...
    The mapping must be **injective**, because two ids sharing a path is a
    credential-loss bug, not a cosmetic one: the second identity would read the
    first one's key, fail to authenticate, re-register, and overwrite the file —
    leaving the first identity's credential unrecoverable, and unrecoverable is
    terminal here (``POST /agents/register`` refuses a duplicate id and rotation
    needs the key you just lost).

    The API does not restrict agent ids to the filename-safe character set, so
    two rules cover the whole input space:

      * an id that is **already filename-safe** — ``[A-Za-z0-9._-]``, at most
        128 characters — keeps its historical ``<agent-id>.key`` name, so
        existing credential files stay valid;
      * **any other id** gets a readable sanitized prefix, a ``~`` marker, and
        128 bits of SHA-256 taken over the *complete* original id.

    The digest covers the whole id, so two ids that differ only past the
    truncation point — or only in characters that sanitize to the same
    replacement — still land on different files. ``~`` is deliberately outside
    the safe charset, so a derived name can never collide with a safe id's name
    either; the two forms are disjoint by construction rather than by luck.
    """
    base = state_dir if state_dir is not None else resolve_state_dir()
    if not agent_id or not agent_id.strip():
        _refuse("cannot derive a credential filename from an empty agent id")

    if _FILENAME_SAFE_ID.fullmatch(agent_id):
        name = f"{agent_id}.key"
    else:
        digest = hashlib.sha256(agent_id.encode("utf-8")).hexdigest()
        prefix = re.sub(r"[^A-Za-z0-9._-]", "_", agent_id)[:_DERIVED_PREFIX_LEN]
        name = f"{prefix}{_DERIVED_MARKER}{digest[:_DERIVED_DIGEST_CHARS]}.key"

    # Belt and braces. Neither branch can emit a path separator — the safe
    # charset excludes them and the other branch substitutes them away — and
    # appending ".key" means the component is never "." or "..". Assert it
    # anyway: this function decides where a credential is written, so the
    # "stays inside the state directory" property is checked, not assumed.
    if os.sep in name or (os.altsep and os.altsep in name):
        _refuse(f"derived credential filename {name!r} is not a single path component")
    return base / name
