...
`iqx.helpers.state.resolve_state_dir()`, in a file named after the resolved
agent id (`<agent-id>.key`) — deriving the filename from the id is what keeps a
cached credential from being handed to a different identity.
